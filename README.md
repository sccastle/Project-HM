...
**更新 App：** 改完代码后，先改两个地方的版本号——`js/version.js` 里的 `version` 和 `service-worker.js` 顶部的 `VERSION`（后者写成 `hm-v` 加前者，例如 `1.5.2` 对 `hm-v1.5.2`），然后**必须跑一次 `node build-single-file.js`**再上传：这一步除了打包单文件，还会把 `?v=版本号` 的戳写进 `index.html` 和 `service-worker.js` 的资源路径里。没有这个戳，GitHub Pages 给静态文件带的缓存头会让浏览器在一段时间内继续用旧的 js，页面上就会出现「代码改了、版本号还是上一版」。两个版本号不一致时构建脚本会直接报错拦下。

上传后手机上把 App 关掉重开。新的 Service Worker 装好接管时，页面会自动刷新一次。还是不对的话，进 设置 → 关于，那里有「检查更新」和「清缓存重载」两个按钮，后者会清掉所有缓存的程序文件并注销 Service Worker（**作业数据不受影响**，数据存在 IndexedDB 里，和缓存是两回事）。
...
底部只有三个 tab：作业 / 知识库 / 设置。日历不再单独占一个 tab，它折叠在作业页里，平时收起，点「按日历选日期」才展开，选完日期自动收起，下面就是那天的作业。老的 `#/calendar` 链接会自动跳到作业页并展开日历。

**阶段性作业**（跨多天、有截止日期的任务，比如 PBL 的项目）置顶在作业页最上面，显示「还有 34 天 / 明天截止 / 今天截止 / 已过期 N 天」，越近的排越前。

设置方式有两条路，都保留：**AI 整理完之后，在今日清单里长按那一条 → 设为阶段性作业 → 选截止日期**（截止日期有一周后 / 两周后 / 一个月后的快捷选项）；或者「手动添加一条」时打开「阶段性作业」开关，默认关闭。电脑上右键等价于长按。

设成阶段性之后，它在清单里就不再显示勾选框（换成星标），因为它已经置顶了——**长按置顶项**会弹出：完成这项阶段性作业 / 修改截止日期 / 取消置顶（变回普通作业）/ 编辑内容 / 删除。它存在被布置那天的记录里，只是多了一个 `due` 字段，所以导出导入、勾选、编辑全都沿用原来那套逻辑。从布置日到截止日之间的每一天，打开这门课都能在清单顶部看到它，在哪天勾选都会写回它原本所在的那条记录。
...
→ 月底进「知识」→ 选 9/1–9/30 → 数学看整月知识脉络，英语看整月单词 → 一键复制
```
...
DailyRecord { id: "2026-09-06__c_xxx", date, gradeId, courseId,
              rawText,                              // 老师原文，事实源
              tasks: [{ id, text, type, completed, due? }]
              // type: student | parent | reminder
              // due:  可选，有值就是阶段性作业，格式 2026-10-10
              createdAt, updatedAt }
...
**知识库分两块。** 数学是知识脉络（按分类聚合知识点），英语是单词（按日期聚合去重的单词）。两块共用同一个时间范围选择器（本周 / 本月 / 自定义），各自有一键复制。英语这块把英语课和 PBL 课的提词结果合在一起，同一个词只出现一次，重复出现的在右上角标次数，按首次出现日期排在对应那天下面。PBL 的核心表达不进知识库，留在课程页里。

**提词只要单个单词。** 提示词里要求词组必须拆开（good morning 出两个词），返回后前端再本地拆一次兜底：按非字母字符切分，词内连字符和撇号保留（T-shirt、don't 仍算一个词），大小写不敏感去重，统一小写（I 除外）。
