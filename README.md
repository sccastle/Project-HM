...
**阶段性作业**（跨多天、有截止日期的任务，比如 PBL 的项目）置顶在作业页最上面，显示「还有 34 天 / 明天截止 / 今天截止 / 已过期 N 天」，越近的排越前。添加或编辑任务时有一个「阶段性作业」开关，默认关闭，打开后才让你选截止日期。它存在被布置那天的记录里，只是多了一个 `due` 字段，所以导出导入、勾选、编辑全都沿用原来那套逻辑。从布置日到截止日之间的每一天，打开这门课都能在清单顶部看到它，在哪天勾选都会写回它原本所在的那条记录。
